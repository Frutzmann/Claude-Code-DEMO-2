...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/components/portraits/portrait-card.tsx"
      provides: "Individual portrait with actions including label edit"
      min_lines: 40
    - path: "src/components/portraits/portrait-upload-dialog.tsx"
      provides: "Upload dialog with drag-drop"
      min_lines: 50
    - path: "src/components/portraits/edit-label-dialog.tsx"
      provides: "Dialog to edit existing portrait labels"
      min_lines: 30
  key_links:
    ...
    - from: "src/components/portraits/portrait-card.tsx"
      to: "updatePortraitLabel"
      via: "edit label button triggers dialog which calls server action"
      pattern: "updatePortraitLabel"
---

<objective>
Build the portraits page UI with grid layout, upload dialog, and card components for managing portrait library.

Purpose: Users can visually manage their portrait collection - view, upload, set active, edit labels, and delete portraits.
...
- Dialog: Upload modal and edit label modal
...
- Hover overlay with action buttons:
  - "Set Active" button (Star icon, hidden if already active)
  - "Edit Label" button (Pencil icon) - triggers onEditLabel callback
  - "Delete" button (Trash2 icon, disabled if isOnlyPortrait)
...
- Call server actions from @/actions/portraits (including updatePortraitLabel)
...
- Show EditLabelDialog when edit label is triggered
- State for editingPortrait: { id: string, label: string } | null
...
**5. edit-label-dialog.tsx (Client Component)**
- Dialog for editing portrait label
- Props: open, onOpenChange, portraitId, currentLabel, onSave
- Input field pre-filled with currentLabel
- Save and Cancel buttons
- On save: call updatePortraitLabel server action, close dialog, show success toast
- Disable save button while pending (use useTransition)

**6. portraits/page.tsx (Server Component)**
...
**7. portraits/client.tsx (Client Component)**
...
- Import icons from lucide-react (Star, Trash2, Plus, Loader2, Upload, Pencil)
- Use sonner toast for notifications
  ...
  <done>All portrait components created with proper imports and typing, including label edit functionality wired to updatePortraitLabel server action</done>
</task>
...
6. Edit label dialog opens when clicking Pencil icon
7. Label edits persist after save
8. Sidebar Portraits link is enabled
</verification>
...
- [ ] User can edit portrait label via Pencil icon and dialog
