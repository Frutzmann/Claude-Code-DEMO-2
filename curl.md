# Test du Webhook YouTube Thumbnail Factory

Script bash pour tester le workflow via curl.

## Usage

```bash
cd "/Users/frutz/Desktop/Presta/Youtube/Claude Code DEMO 2"
bash -c "$(cat curl.md | sed -n '/^```bash$/,/^```$/p' | sed '1d;$d' | tail -n +2)"
```

Ou copier/coller le script ci-dessous dans le terminal.
...
Le webhook attend un JSON avec cette structure:

```json
{
  "Keywords": "visage centre au milieu;gauche:screenshot workflow...",
  "Background Images": [
    {
...
- Le webhook repond immediatement (mode `responseNode`)
- Le traitement continue en arriere-plan
- Les thumbnails generees seront stockees dans Airtable
- Timeout de 5 minutes maximum par tache de generation
